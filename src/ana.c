...
int Adriana( char * NetFileName, char * InvFileName, int SolverNum, int ty, float ap, int numprocs, int debug )
{   
...
// aggregate clans
  if( ap > 0.0 )
  {
    i = AggregateClansMETIS( NetFileName, nz, numprocs-1, (int)ap, debug ); /* agg_clans_METIS */
    if(debug>0)printf("# Aggregated by METIS into %d clans\n", i);
    nz=i; 
  }
  else if( ap < 0.0 )
  {
    nz = AggregateClans( NetFileName, nz, numprocs-1, -ap, debug ); /* agg_clans */
    if(debug>0)printf("# Aggregated into %d clans\n", nz); 
  }
...
if(debug>0)printf("# Solve a system for each clan\n");
...
int CleanWorkFiles( int z, char * NetFileName )
{
  ...
  remove( fname );
  sprintf( fname, "%s.d", NetFileName );
  remove( fname );
  sprintf( fname, "%s.metis", NetFileName );
  remove( fname );
