...
"               infile outfile\n"
"FLAGS          WHAT\n"
"np             number of processes (nodes)\n"
"-h             print help (this mode)\n"
"-c             simultaneous composition of clans\n"
"-s             parallel-sequential composition of clans\n"
"-r name        solver name (ParTou, zsolve) or a new name (default ParTou)\n"
"-T             transpose matrix\n"
"-a ap          aggregate clans:\n"
"               (a) `ap>0` -- METIS: `ap=1` -- N-1 bins, `ap>1` -- ap partitions;\n"
"               (b) `ap<0` -- bin packing: `ap=-1` -- N-1 bins, `ap<-1` -- bin size ap*max_clan size;\n"
"infile         input file\n"
"outfile        output file\n\n"
...
if(debug_level>2) printf("# i am master %d of %d run on %s\n", rank, numprocs, processor_name); 
    ...
    for( i=1; i<argc; i++ )
    {
      ...
      else if( strcmp( argv[i], "-a" )==0 ) { snumf=numf; numf=-4; }
      else if( numf==-4 ) { ap=atof( argv[ i ] ); numf=snumf; }
...
if(debug_level>2) printf("# solvername %s, solvernum %d\n", solver_name_array[SolverNum], SolverNum);
    ...
    Adriana( fname1, SolutionFileName, SolverNum, mode, ap, numprocs, debug_level ); // solve a given system
